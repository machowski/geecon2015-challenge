...
import pl.allegro.promo.geecon2015.domain.transaction.DefaultUserTransactions;
import pl.allegro.promo.geecon2015.domain.transaction.MissingUserTransactions;
...
public class RestTransactionRepository implements TransactionRepository {
    ...
    public UserTransactions transactionsOf(UUID userId) {
        try {
            return restTemplate.getForEntity(baseUri + "/transactions/" + userId.toString(), DefaultUserTransactions.class).getBody();
        } catch (RestClientException e) {
            return new MissingUserTransactions();
        }
    }
}
